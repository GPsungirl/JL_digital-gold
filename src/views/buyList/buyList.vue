  ...
  <div class="pad_5">
    ...
    <div>
      ...
      <el-table :row-class-name="tableRowClassName" :data="tableData" v-loading="tableLoading" border  style="width: 100%">
        ...
        <el-table-column prop="" label="状态" width="260">
          <template slot-scope="scope">
            <span v-if="scope.row.pay_status == 1">已付款</span>
            <span v-if="scope.row.amount_status == 0">未放币</span>
            <span v-if="scope.row.amount_status == 1">已放币</span>
        ...
        <el-table-column prop="" label="放币状态" width="">
          <template slot-scope="scope">
            <span v-if="scope.row.amount_status == 0">未放币</span>
            <span v-if="scope.row.amount_status == 1">已放币</span>
          </template>
        </el-table-column>
        <el-table-column prop="" label="预付状态" width="">
          <template slot-scope="scope">
            <span v-if="scope.row.pre_amount_status == 0">未返</span>
            <span v-if="scope.row.pre_amount_status == 1">已返</span>
        ...
        <el-table-column prop="" label="终止状态" width="">
          <template slot-scope="scope">
            <span v-if="scope.row.deal_status == 1">未终止</span>
            <span v-else>已终止</span>
          </template>
    ...
    <el-dialog
      ...
      >
        ...
        <el-form-item label="数字钱包" prop="account_no">
...
export default {
  ...
  methods: {
    ...
    getTableDataList(pageNum) {
      ...
      this.$http
        .post(`${commonUrl.baseUrl}/orderInfo/buyList`, param)
        .then(res => {
          if (res.data.code == "0000") {
            this.tableLoading = false;
            this.tableData = res.data.data.buyOrderList;
            // 分页 总数
